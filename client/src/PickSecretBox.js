...
function PickSecretBox() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch('https://pick-server-zeta.vercel.app/wishlist/pick');
        if (!response.ok) {
  ...
  const pickName = async (id) => {
    const response = await fetch(`https://pick-server-zeta.vercel.app/wishlist/pick/${id}`);
    const data = await response.json();
